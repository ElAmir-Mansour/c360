...
/* ------------------------------------------------------------------------- *
 * FEATURE 5 — Matter document links.
 *
 * Self-contained module for the matter detail page. Lists existing
 * matter <-> LegalDocument join rows, links an existing lex document via a
 * search-backed picker (POST {document_id, relationship}), and unlinks a link
 * (the underlying LegalDocument is WORM and never deleted).
 *
 * fe-client (api.ts / suites.ts) owns the shared type + client methods; this
 * file references the agreed method names:
 *   enterpriseApi.lex.listMatterDocuments(matterId)            -> LexMatterDocumentLink[]
 *   enterpriseApi.lex.addMatterDocument(matterId, payload)     -> LexMatterDocumentLink
 *   enterpriseApi.lex.removeMatterDocument(matterId, linkId)   -> void
 *
 * The join shape is mirrored locally so the module compiles independently of
 * the fe-client edit ordering; once suites.ts exports `LexMatterDocumentLink`
 * the page can pass the same shape through verbatim.
 * ------------------------------------------------------------------------- */

/** A matter <-> LegalDocument join row (mirrors the FEATURE 5 backend contract). */
...
/** POST body for linking an existing document to a matter. */
export interface AddMatterDocumentPayload {
  document_id: string;
  relationship?: string;
}

// The fe-client api surface gains these methods in parallel. They are typed
// here against the agreed signatures so this module stays self-contained.
type MatterDocumentApi = {
  listMatterDocuments: (matterId: string) => Promise<MatterDocumentLink[]>;
  addMatterDocument: (
    matterId: string,
    payload: AddMatterDocumentPayload,
  ) => Promise<MatterDocumentLink>;
  removeMatterDocument: (matterId: string, linkId: string) => Promise<void>;
};

function matterDocumentApi(): MatterDocumentApi {
  return enterpriseApi.lex as unknown as typeof enterpriseApi.lex & MatterDocumentApi;
}

/** Relationship vocabulary for a matter document link (default: "related"). */
...
interface MatterDocumentsLabels {
  title: string;
  description: string;
  link: string;
  loading: string;
...
const matterDocumentsLabelsBundle: LexBilingual<MatterDocumentsLabels> = {
  en: {
    title: 'Linked Documents',
    description: 'Documents from the repository linked to this matter.',
    link: 'Link document',
    loading: 'Loading linked documents…',
    loadError: 'Failed to load linked documents.',
    emptyTitle: 'No linked documents',
    emptyDescription: 'No repository documents are linked to this matter yet.',
    ...
    cancel: 'Cancel',
    toastLinked: 'Document linked to matter.',
    toastUnlinked: 'Document unlinked.',
    dialogTitle: 'Link document',
    dialogDescription: 'Search the document repository and link an existing document to this matter.',
    ...
    alreadyLinkedHint: 'Documents already linked to this matter are hidden.',
  ...
  ar: {
    ...
    description: 'وثائق من المستودع مرتبطة بهذه القضية.',
    link: 'ربط وثيقة',
    loading: 'جارٍ تحميل الوثائق المرتبطة…',
    loadError: 'تعذّر تحميل الوثائق المرتبطة.',
    emptyTitle: 'لا توجد وثائق مرتبطة',
    emptyDescription: 'لا توجد وثائق من المستودع مرتبطة بهذه القضية بعد.',
    ...
    cancel: 'إلغاء',
    toastLinked: 'تم ربط الوثيقة بالقضية.',
    toastUnlinked: 'تم إلغاء ربط الوثيقة.',
    dialogTitle: 'ربط وثيقة',
    dialogDescription: 'ابحث في مستودع الوثائق واربط وثيقة قائمة بهذه القضية.',
    ...
    alreadyLinkedHint: 'تُخفى الوثائق المرتبطة مسبقًا بهذه القضية.',
...
export function MatterDocumentsSection({ matterId, canWrite }: MatterDocumentsSectionProps) {
  const { locale, direction } = useLocaleOrDefault();
  const labels = useMemo(() => resolveLexBilingual(matterDocumentsLabelsBundle, locale), [locale]);
  const queryClient = useQueryClient();
  const api = matterDocumentApi();

  const [linkOpen, setLinkOpen] = useState(false);
  const [unlinkTarget, setUnlinkTarget] = useState<MatterDocumentLink | null>(null);
  const [downloadingId, setDownloadingId] = useState<string | null>(null);

  const linksQuery = useQuery({
    queryKey: ['lex-matter-documents', matterId],
  ...
  const linkMutation = useMutation({
    mutationFn: (payload: AddMatterDocumentPayload) => api.addMatterDocument(matterId, payload),
    onSuccess: async () => {
      showSuccess(labels.toastLinked);
      setLinkOpen(false);
      await queryClient.invalidateQueries({ queryKey: ['lex-matter-documents', matterId] });
    },
    onError: showApiError,
  });

  const unlinkMutation = useMutation({
    mutationFn: (linkId: string) => api.removeMatterDocument(matterId, linkId),
    onSuccess: async () => {
...
interface MatterLinkDocumentDialogProps {
  open: boolean;
  loading: boolean;
  labels: MatterDocumentsLabels;
  linkedDocumentIds: Set<string>;
  onOpenChange: (open: boolean) => void;
  onSubmit: (payload: AddMatterDocumentPayload) => void;
}

function MatterLinkDocumentDialog({
  open,
  loading,
  labels,
  linkedDocumentIds,
  ...
  const documentsQuery = useQuery({
    queryKey: ['lex-documents', 'matter-link-picker'],
    queryFn: () =>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg" dir={direction} lang={locale}>
        ...
        {documentsQuery.isError ? (
          <ErrorState message={labels.pickerLoadError} onRetry={() => void documentsQuery.refetch()} />
        ) : (
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="matter-doc-search">{labels.document}</Label>
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger aria-label={labels.document}>
                  <SelectValue
                    placeholder={
                      documentsQuery.isLoading ? labels.loadingDocuments : labels.documentPlaceholder
                    }
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="matter-doc-relationship">{labels.relationship}</Label>
              <Select
                value={relationship}
                onValueChange={(value) => setRelationship(value as DocumentRelationship)}
              >
                <SelectTrigger id="matter-doc-relationship">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
