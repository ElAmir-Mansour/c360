...
/**
 * Tenant-wide event inspector — `/lex/admin/integrations/events`.
 *
 * A cross-endpoint view of every inbound/outbound event for this tenant (#17),
 * rendered through the shared {@link EventInspector} in its global mode (adds an
 * Integration column). The integrations registry is loaded alongside purely to
 * label the Integration column with human-readable names instead of raw ids; if
 * that lookup is unavailable, events stay visible by raw id with a warning.
 *
 * Reads gated on `lex:integration:read`; per-event replay requires
 * `lex:integration:manage` (coarse `lex:write` also satisfies manage). Mirrors
 * the tenant-wide DLQ page: PermissionRedirect, back link, PageHeader,
 * SectionCard, bilingual labels (Arabic-first), react-query, RTL.
 */
    ...
    <PermissionRedirect permission="lex:integration:read">
      <div dir={direction} lang={locale} className="space-y-6">
        <div>
          ...
          <PageHeader eyebrow={t.eventsTab} title={t.eventsGlobalTitle} description={t.eventsGlobalSubtitle} />
        </div>

        <SectionCard title={t.eventsGlobalTitle} description={t.eventsGlobalSubtitle}>
          {registryDegraded ? (
          ...
          <EventInspector canManage={canManage} endpointNames={endpointNames} />
        </SectionCard>
      </div>
    </PermissionRedirect>
  );
