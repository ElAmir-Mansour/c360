-- Migration 106 created the manager-task aggregate. Harden its policies for
-- pooled connections: SET LOCAL leaves a custom GUC as an empty string after
-- transaction end, so NULLIF is required for fail-closed UUID casts.
