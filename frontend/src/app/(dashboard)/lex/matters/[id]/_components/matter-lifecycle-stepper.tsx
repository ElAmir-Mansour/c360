...
/**
 * #4 Lifecycle stepper — a COMPACT, wrapping, audit-grade view of the matter
 * FSM happy path (intake → open → in_review → closed).
 *
 * Design mirrors the Legal Service Desk request stepper:
 *  - A single wrapping row of small state marks: completed = check in a filled
 *    dot, current = filled pulsing dot, future = hollow dot. Concise stage label
 *    beside each dot.
 ...
 *  - Timing from the append-only matter audit (`listMatterAudit`): each stage is
 *    stamped with the timestamp it was ENTERED (earliest transition whose
 *    `to_status` maps into that stage; `waiting_on_business` collapses onto the
 *    "open" stage). The CURRENT stage shows an inline "entered {relative}"
 *    caption; COMPLETED stages expose the absolute dual date + dwell duration on
 *    hover. If audit is empty or fails to load the stepper still renders — just
 *    without timing.
 *  - Off-path `on_hold` / `cancelled`: a header chip + the terminal hint, and
 *    (when audit is available) the stages actually reached are marked as visited.
 *
 * The matter audit stream is empty for many matters today (the backend does not
 * yet emit audit events for every matter), which this tolerates gracefully.
 */
...
import {
  useMatterStepperLabels,
  type MatterStepperTimingUnits,
} from './matter-detail-labels';

/** Ordered happy-path step keys. */
const STEP_KEYS = ['intake', 'open', 'in_review', 'closed'] as const;
type StepKey = (typeof STEP_KEYS)[number];

/** Map a raw matter status onto its happy-path step index. `waiting_on_business`
 * collapses onto the "open" working stage; off-path `on_hold` / `cancelled`
 * return -1. */
...
export interface MatterLifecycleStepperProps {
  status: string;
  matterId: string;
}

export function MatterLifecycleStepper({ status, matterId }: MatterLifecycleStepperProps) {
  const t = useMatterStepperLabels();
  ...
  // Append-only audit — shared cache key with the activity mini + full feed.
  ...
  // Per happy-path stage: the earliest timestamp it was ENTERED, plus when (if
  // ever) the matter went off-path.
  const { enteredAt, terminalAt } = useMemo(() => {
    const map = new Map<number, number>();
    let terminal: number | null = null;
    const entries: LexMatterAuditEntry[] = auditQuery.data ?? [];
  ...
  /** Dwell time in a completed stage = gap until the next stage with a recorded
   * entry timestamp. Returns null when not derivable. */
