...
func (r *fakeComparisonRunner) AggregateAllShadowModels(context.Context) error {
	r.calls++
	return r.err
}
...
func TestSchedulerReturnsCanceledWithoutRetryingCanceledRun(t *testing.T) {
	runner := &fakeComparisonRunner{err: fmt.Errorf("begin read-only transaction: %w", context.Canceled)}
...
func TestSchedulerTreatsClosedPoolAsCanceled(t *testing.T) {
	runner := &fakeComparisonRunner{err: errors.New("begin read-only transaction: closed pool")}
