-- Refresh only untouched platform-default templates seeded before the Clario
-- palette migration. Matching the complete previous body protects customized
-- platform defaults; tenant-specific templates are excluded by tenant_id.
UPDATE notification_templates AS template
SET body_tmpl = replacement.new_body,
    updated_at = now()
FROM (VALUES
...
WHERE template.id = replacement.id
  ...
  AND template.body_tmpl = replacement.old_body;
